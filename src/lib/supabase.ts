...
export const createSignal = async (signalData: any) => {
  try {
    ...
    // Check if we should create an event (5+ signals in same location within 24h)
    await checkAndCreateEvent(signalData.location, signalData.type);
...
// Function to check if we should create an event based on signal count
const checkAndCreateEvent = async (location: string, type: string) => {
  try {
    ...
    console.log(`Found ${signalCount} signals in ${location} within 24h`);
    
    // If we have 5 or more signals, create an event
    if (signalCount >= 5) {
      ...
      if (!existingEvents || existingEvents.length === 0) {
        // Create new event
        const severity = signalCount >= 15 ? 'high' : signalCount >= 10 ? 'medium' : 'low';
        
        const eventData = {
          title: `${type} cluster detected - ${location}`,
          location: location,
          event_type: type,
          severity: severity,
          status: 'active',
          signal_count: signalCount,
          description: `Unusual spike in ${type} signals detected in ${location} area. ${signalCount} signals reported within 24 hours.`
        };
        ...
        // Create blog post
        const blogData = {
          title: `Health Alert: ${type} signals spike in ${location}`,
          content: `Our monitoring system has detected an unusual increase in ${type} signals in the ${location} area. This early warning indicates potential health concerns that warrant attention and preventive measures. 
          
          **What we detected:**
          - ${signalCount} ${type} signals in 24 hours
          ...
          **Recommended actions:**
          ...
          This is an automated alert generated by our AI monitoring system. Local health authorities have been notified.`,
          author: 'Prevora AI System',
          published: true,
        ...
        if (blogError) {
          ...
          console.log('Blog post created for event');
        }
      }
    }
  } catch (error) {
