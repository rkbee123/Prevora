/*
  # Enhanced Event Creation and Signal Processing Functions

  1. Functions
    - `create_event_from_signals()` - Creates events when signal threshold is reached
    - `update_event_signal_count()` - Updates signal counts for existing events
    - `auto_resolve_old_events()` - Auto-resolves old events with no recent activity

  2. Triggers
    - Signal insertion triggers for event creation and count updates

  3. Indexes
    - Performance indexes for location fuzzy matching and date filtering

  4. Sample Data
    - Test signals to demonstrate the system functionality
*/
...
*Generated by Prevora AI Prevention Network - Detecting health threats before they spread.*',
        ...
        ELSE
            -- Update existing event signal count
            UPDATE events 
