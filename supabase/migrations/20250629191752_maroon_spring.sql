...
*Generated by Prevora AI Prevention Network - Detecting health threats before they spread.*',
        ...
        ELSE
            ...
            SET signal_count = signal_count + 1,
                description = description || ' Updated: ' || TO_CHAR(NOW(), 'HH24:MI') || ' - Additional signal detected.'
            WHERE (
                LOWER(location) LIKE '%' || LOWER(SPLIT_PART(NEW.location, ',', 1)) || '%'
                OR LOWER(SPLIT_PART(NEW.location, ',', 1)) LIKE '%' || LOWER(SPLIT_PART(location, ',', 1)) || '%'
            )
...
BEGIN
    ...
    description = CASE 
        ...
        THEN description || ' [Updated: ' || TO_CHAR(NOW(), 'HH24:MI') || ']'
        ELSE description
    END
    WHERE (
        LOWER(location) LIKE '%' || LOWER(SPLIT_PART(NEW.location, ',', 1)) || '%'
...
BEGIN
    ...
    UPDATE events 
    SET status = 'resolved',
        description = description || ' [Auto-resolved: No recent signals for 72+ hours]'
    WHERE status = 'active'
    AND created_at < seventy_two_hours_ago
    AND NOT EXISTS (
    ...
    SET status = 'resolved'
    WHERE status = 'active'
    AND issued_at < seventy_two_hours_ago
    AND EXISTS (
